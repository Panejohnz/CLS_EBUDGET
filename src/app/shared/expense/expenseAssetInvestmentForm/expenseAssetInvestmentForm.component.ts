...
export class ExpenseAssetInvestmentFormComponent {
  ...
  loadExpenseRates() {
    ...
    forkJoin(requests)
        ...
        this.Mas_Expense_Detial_Rate_List =
          responses.reduce((list: any[], response: any, index: number) => {
            ...
            return Array.isArray(expenseRateList)
              ? list.concat(expenseRateList.map((rate: any) => ({
                ...rate,
                __Expense_Detail_Id: expenseIds[index]
              })))
              : list;
          }, []);
      ...
      console.log('Mas_Expense_Detial_Rate_List', this.Mas_Expense_Detial_Rate_List);
  }
  ...
  private getRateExpenseDetailId(row: any): any {
    return row?.__Expense_Detail_Id ??
      row?.Fk_Expense_Detail_Id ??
      row?.FK_Expense_Detail_Id ??
      row?.Fk_Expense_Detial_Id ??
      this.getExpenseDetailId(row);
  }
